...
let SongsListMock: React.FC<Record<string, unknown>> = (props) => {
  if (props.isLoading) return <div>Loading search results...</div>;
  if (props.error) return <div>Error loading search results.</div>;
...
const mockUseGetSongsQuery = useGetSongsQuery as jest.Mock;
...
describe('SearchPage', () => {
  beforeEach(() => {
    ...
    mockUseGetSongsQuery.mockReturnValue({
  ...
  it('renders header, search input, and songs list', () => {
    render(<SearchPage />);
    expect(screen.getByTestId('page-header')).toHaveTextContent('Search Music');
    expect(screen.getByTestId('search-input')).toBeInTheDocument();
    expect(screen.getByTestId('songs-list')).toHaveTextContent('Songs');
  });

  it('shows loading state', () => {
    mockUseGetSongsQuery.mockReturnValue({ isLoading: true });
    render(<SearchPage />);
    expect(screen.getByText(/loading search results/i)).toBeInTheDocument();
  });

  it('shows error state', () => {
    mockUseGetSongsQuery.mockReturnValue({ isLoading: false, error: 'Error' });
    render(<SearchPage />);
  ...
  it('resets page to 1 when search changes', () => {
    ...
    const input = screen.getByTestId('search-input') as HTMLInputElement;
    fireEvent.change(input, { target: { value: 'rock' } });
    // The component should reset currentPage to 1 when search changes
    expect(mockUseGetSongsQuery).toHaveBeenCalledWith(
      {
        searchTerm: 'rock',
        page: 1,
      },
  ...
  it('handles initial search from URL params', () => {
    mockUseSearchParams.mockReturnValue({
      get: (key: string) => (key === 'search' ? 'initial search' : '1'),
    });
    render(<SearchPage />);
    const input = screen.getByTestId('search-input') as HTMLInputElement;
